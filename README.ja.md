...
> [!IMPORTANT]
> 配布物はAppleの**公証（notarization）を受けていません**（本プロジェクトは意図的にApple Developer Programへ加入していません）。そのため初回起動時に「開発元を確認できません」や「壊れているため開けません」といった警告が表示されます。**アプリは壊れていません** — Gatekeeperが検証できないダウンロードに出す標準の反応です。次のどちらかで解除できます。
>
> ```bash
> xattr -cr /Applications/CodexLens.app
> ```
>
> または **システム設定 → プライバシーとセキュリティ** を開いて下までスクロールし、**「このまま開く」** をクリックしてください。
>
> この操作は初回の1回だけです。すべてのDMGはこのリポジトリから[GitHub Actions](.github/workflows/release.yml)でビルドされており、中身は誰でも監査できます。
...
ビルドが未署名のためです（本プロジェクトはApple Developer証明書を使用していません）。[インストール手順](#アプリ版をダウンロード推奨)にある2クリックの回避方法をご覧ください。必要なのは初回の1回だけです。
...
- リリースは未署名のため、初回起動時にGatekeeperの警告が1度表示されます（詳細は[配布ドキュメント](docs/distribution.md)）。
